...
import { getDecoderDocument, listDecoderDocuments } from "@/lib/decoder-store";
import { isReviewerUnlocked } from "@/lib/reviewer-auth";
import { sanitizeDocumentDetail } from "@/lib/sensitive-documents";
import type { DecoderDocumentDetail, DecoderFact } from "@/lib/decoder-types";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const unlocked = isReviewerUnlocked(request);
    const sanitizedDocument = sanitizeDocumentDetail(best.document, unlocked);
...
function pickAnswerFact(question: string, facts: DecoderFact[]) {
  ...
  const ranked = facts
    .map((fact) => {
      ...
      if (credentialQuestion && includesAny(labelText, ["password", "credential"])) {
        score += 20;
      }
      if (credentialQuestion && includesAny(labelText, ["wifi", "wi-fi", "network"])) {
        score += 6;
      }
...
function buildAnswer(
  question: string,
  document: DecoderDocumentDetail,
  fact: DecoderFact | undefined | null
) {
  ...
  if (!value) {
    return `I found a likely source: ${title}. Open the proof to confirm the exact answer.`;
  }
  ...
  if (looksLikeCredentialQuestion(question)) {
    return `${label}: ${value}. Source: ${title}.`;
  }
...
function duplicateKey(document: DecoderDocumentDetail) {
  ...
  const credentialValues = document.facts
    .filter((fact) => fact.fact_value && includesAny(`${fact.fact_type} ${fact.label ?? ""}`, ["password", "credential", "wifi", "wi-fi"]))
